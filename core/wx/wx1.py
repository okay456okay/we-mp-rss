...
class MpsApi(WxGather):
    ...
    def get_Articles(self, faker_id:str=None,Mps_id:str=None,Mps_title="",CallBack=None,start_page=0,MaxPage:int=1,interval=10,Gather_Content=True,Item_Over_CallBack=None,Over_CallBack=None):
        ...
        if self.Gather_Content:
             Gather_Content=True
        logger.info(f"[文章获取-API模式] 开始获取文章 - 公众号: {Mps_title}, 公众号ID: {Mps_id}, faker_id: {faker_id}, 起始页: {start_page}, 最大页: {MaxPage}, 采集内容: {Gather_Content}")
        ...
        while True:
            if i >= MaxPage:
                logger.info(f"[文章获取-API模式] 达到最大页数，停止获取 - 公众号: {Mps_title}, 当前页: {i}, 最大页: {MaxPage}")
                break
            begin = i * count
            params["begin"] = str(begin)
            logger.info(f"[文章获取-API模式] 开始获取第{i+1}页 - 公众号: {Mps_title}, begin: {begin}")
            print(f"第{i+1}页开始爬取\n")
            # 随机暂停几秒，避免过快的请求导致过快的被查到
            sleep_time = random.randint(0,interval)
            logger.debug(f"[文章获取-API模式] 等待 {sleep_time} 秒后继续 - 公众号: {Mps_title}")
            time.sleep(sleep_time)
            try:
                headers = self.fix_header(url)
                # 记录请求详细信息
                logger.debug(f"[文章获取-API模式] 发送请求 - 公众号: {Mps_title}, URL: {url}, 参数: {params}")
                ...
                logger.debug(f"[文章获取-API模式] 收到响应 - 公众号: {Mps_title}, ret: {ret_code}, err_msg: {err_msg}, 状态码: {resp.status_code}")

                self._cookies=resp.cookies
                
                # 如果返回错误，记录完整的请求和响应信息
                if ret_code != 0:
                    logger.error(f"[文章获取-API模式] API返回错误 - 公众号: {Mps_title}, 请求URL: {url}")
                    logger.error(f"[文章获取-API模式] 请求参数: {params}")
                    logger.error(f"[文章获取-API模式] 响应内容: {msg}")
                    logger.error(f"[文章获取-API模式] HTTP状态码: {resp.status_code}")
                
                # 流量控制了, 退出
                if ret_code == 200013:
                    logger.error(f"[文章获取-API模式] 触发频率限制 - 公众号: {Mps_title}, 在页码 {begin} 处停止")
                    super().Error("frequencey control, stop at {}".format(str(begin)), code="FrequencyControl")
                    break
                
                if ret_code == 200003:
                    logger.error(f"[文章获取-API模式] 登录会话失效 - 公众号: {Mps_title}, 在页码 {begin} 处停止")
                    super().Error("Invalid Session, stop at {}".format(str(begin)),code="Invalid Session")
                    break
                
                # 如果返回的内容中为空则结束
                if 'app_msg_list' not in msg:
                    logger.info(f"[文章获取-API模式] 没有更多文章 - 公众号: {Mps_title}, 在页码 {begin} 处停止")
                ...
                if ret_code != 0:
                    ...
                    logger.error(f"[文章获取-API模式] API返回错误 - 公众号: {Mps_title}, 错误原因: {err_msg}, 错误代码: {ret_code}")
                ...
                if "app_msg_list" in msg:
                    ...
                    for item in msg["app_msg_list"]:
                        ...
                        if Gather_Content:
                            if not super().HasGathered(item["aid"]):
                                logger.debug(f"[文章获取-API模式] 采集文章内容 - 公众号: {Mps_title}, 文章ID: {item['aid']}, 标题: {item.get('title', '未知')[:50]}")
                    ...
                    logger.info(f"[文章获取-API模式] 第{i+1}页获取成功 - 公众号: {Mps_title}, 本页文章数: {page_articles}, 累计文章数: {total_articles}")
                    print(f"第{i+1}页爬取成功\n")
                # 翻页
                i += 1
            except requests.exceptions.Timeout:
                logger.error(f"[文章获取-API模式] 请求超时 - 公众号: {Mps_title}, 页码: {i+1}")
                print("Request timed out")
                break
            except requests.exceptions.RequestException as e:
                logger.error(f"[文章获取-API模式] 请求异常 - 公众号: {Mps_title}, 页码: {i+1}, 错误: {str(e)}")
                print(f"Request error: {e}")
                break
            except Exception as e:
                logger.error(f"[文章获取-API模式] 处理异常 - 公众号: {Mps_title}, 页码: {i+1}, 错误: {str(e)}")
                import traceback
                logger.error(f"[文章获取-API模式] 错误堆栈: {traceback.format_exc()}")
                break
            finally:
                super().Item_Over(item={"mps_id":Mps_id,"mps_title":Mps_title},CallBack=Item_Over_CallBack)
        logger.info(f"[文章获取-API模式] 获取完成 - 公众号: {Mps_title}, 总共获取文章数: {total_articles}")
