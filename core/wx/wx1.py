    ...
    def get_Articles(self, faker_id:str=None,Mps_id:str=None,Mps_title="",CallBack=None,start_page=0,MaxPage:int=1,interval=10,Gather_Content=True,Item_Over_CallBack=None,Over_CallBack=None):
        ...
        while True:
            ...
            try:
                ...
                logger.debug(f"[文章获取-API模式] 发送请求 - 公众号: {Mps_title}, URL: {url}, 参数: begin={begin}, fakeid={faker_id}")
                resp = session.get(url, headers=headers, params = params, verify=False)
                
                msg = resp.json()
                logger.debug(f"[文章获取-API模式] 收到响应 - 公众号: {Mps_title}, ret: {msg.get('base_resp', {}).get('ret', 'unknown')}")
                ...
                if msg['base_resp']['ret'] != 0:
                    ...
                    logger.error(f"[文章获取-API模式] API返回错误 - 公众号: {Mps_title}, 错误原因: {error_msg}, 错误代码: {error_code}")
